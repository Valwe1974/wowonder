...
////imagesUppld
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      appBar: AppBar(
        actions: [
          if (Activate_Audio_Calls)
            IconButton(
                onPressed: () async {
                  print('ameen');
                  var calls = await ApiCallsAgora.calls(widget.userid, 'audio');
          ...
          if (Activate_Video_Calls)
            IconButton(
              onPressed: () async {
                print('ameen');
                var calls = await ApiCallsAgora.calls(widget.userid, 'video');
                SocketNew.calssSokeit(
                  call_type: 'video',
                  callid: calls['id'],
                  id: widget.userid,
                      ...
                      children: [
                        ...
                        Center(
                          child: Container(
                            width: Get.width * 0.4,
                            height: Get.height * 0.2,
                            child: Image.file(File(ImagePost!.path)),
                          ),
                        )
                      ],
                    ),
